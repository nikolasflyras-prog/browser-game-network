...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (game.status === "diagnostic") {
    return { title: game.title, description: game.description, robots: { index: false, follow: false } };
  }
  ...
  return {
    ...
    openGraph: { type: "website", siteName: SITE_NAME, title: game.title, description: game.description, url: path },
    twitter: { card: "summary", title: game.title, description: game.description },
  };
}
...
export default async function GamePage({ params }: PageProps) {
  ...
  const immersive = game.slug === "semiconductor-vc";
    ...
    <article className={immersive ? "game-page immersive-game-page" : "game-page"}>
      ...
      <header className={immersive ? "game-heading immersive-game-heading" : "game-heading"}>
        <div className="game-heading-meta">
          <p className="eyebrow">{game.collection ?? game.lane}</p>
        ...
        <div className="game-guide">
          ...
          <section className="content-section guide-columns">
            <div>
              ...
              <ul>{seo.concepts.map((concept) => <li key={concept}>{concept}</li>)}</ul>
            </div>
            <div>
              <h2>Strategy</h2>
              <ul>{seo.strategy.map((tip) => <li key={tip}>{tip}</li>)}</ul>
            </div>
          </section>
          <section className="content-section">
