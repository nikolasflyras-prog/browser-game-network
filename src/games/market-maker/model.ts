...
export const marketMakerPostures: readonly QuotePosture[] = [
  "tight",
  "balanced",
  "wide",
  "lean-long",
...
export function playMarketRound(state: MarketMakerState, posture: QuotePosture): MarketMakerState {
  ...
  if (buyFill && sellFill) {
    feedback = "You captured both sides of the spread and finished the round flat to the two customer trades.";
  } else if (!buyFill && !sellFill) {
    feedback = "No flow reached your quotes. Wider pricing can protect you, but it may also earn nothing.";
  } else if (Math.abs(inventory) >= 4) {
    feedback = "Inventory is becoming the main risk. Skew quotes to encourage flow that reduces the position.";
  } else if (posture === "tight") {
    feedback = "Tight quotes attracted flow, increasing both spread opportunity and exposure to informed trading.";
  } else if (posture === "wide") {
    feedback = "Wide quotes reduced fill risk, but you gave up trading opportunities.";
  }
...
export function marketMakerStyle(state: MarketMakerState) {
  if (state.score < 0) return "loss-making" as const;
  if (Math.abs(state.inventory) >= 4) return "inventory-heavy" as const;
  if (Math.abs(state.inventory) <= 1) return "balanced-dealer" as const;
  return "selective" as const;
}

export function marketMakerResult(state: MarketMakerState) {
  if (!state.complete) return null;

  const totalFills = state.history.reduce(
    (sum, round) => sum + Number(round.buyFill) + Number(round.sellFill),
    0,
  );
  const peakInventory = state.history.reduce(
    (peak, round) => Math.max(peak, Math.abs(round.inventoryAfter)),
    0,
  );
  const totalRiskPenalty = state.history.reduce((sum, round) => sum + round.riskPenalty, 0);
  const noFlowRounds = state.history.filter((round) => !round.buyFill && !round.sellFill).length;
  ...
  return {
    ...
    style: marketMakerStyle(state),
    totalFills,
    peakInventory,
    totalRiskPenalty: roundToCent(totalRiskPenalty),
    noFlowRounds,
  } as const;
}
