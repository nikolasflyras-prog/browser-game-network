...
export const futureGameAnalyticsContracts: readonly FutureGameAnalyticsContract[] = [
  ...
  },
  {
    ...
    events: [
      { event: "daily_started", when: "daily seed begins", properties: ["daily_id", "date_key", "seed"] },
      { event: "level_completed", when: "a train route resolves", properties: ["daily_id", "date_key", "turn", "action", "target", "actual", "correct", "strikes"] },
      { event: "game_action", when: "raw switch input resolves", properties: ["daily_id", "date_key", "action", "turn", "switch_action", "target", "actual", "correct", "strikes"] },
      { event: "daily_completed", when: "daily ends", properties: ["daily_id", "date_key", "score", "strikes", "won", "correct_routes", "total_routes", "sequence", "duration_ms", "saved_best"] },
      { event: "game_completed", when: "daily runtime reaches a terminal state", properties: ["mode", "daily_id", "date_key", "score", "won"] },
      { event: "game_restarted", when: "daily is replayed", properties: ["mode", "daily_id", "date_key", "input_type"] },
      { event: "share_clicked", when: "result copy is requested", properties: ["daily_id", "date_key", "score", "streak", "won", "method"] },
    ],
  },
  {
    slug: "market-maker",
