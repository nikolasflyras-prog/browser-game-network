...
grep -q "Learn by living inside the system." "$ARTIFACT_DIR/learn.html"
grep -q "Semiconductors" "$ARTIFACT_DIR/learn.html"
...
echo "Browser smoke test passed: public discovery exposes launch-ready Play/Learn/Semis/Daily copy while hiding diagnostics, desktop/mobile screenshots are captured, all three baseline Phaser runtimes mount, and Orbit Relay passes full input + successful-capture checks."
