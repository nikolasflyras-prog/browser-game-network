...
"$CHROME" "${COMMON_FLAGS[@]}" --window-size=390,844 \
  --screenshot="$ARTIFACT_DIR/system-check-mobile.png" "$BASE_URL/games/system-check" >/dev/null 2>&1

grep -q '<canvas' "$ARTIFACT_DIR/system-check.dom.html"
grep -q '>Pause<' "$ARTIFACT_DIR/system-check.dom.html"
grep -q '>Restart<' "$ARTIFACT_DIR/system-check.dom.html"
...
echo "Browser smoke test passed: routes render, 404 works, Phaser canvas mounts, controls render, and desktop/mobile screenshots were captured."
